...
exports.symbols_to_kline = async (symbols, interval) => {
    ...
    for (let i = 0; i < symbols.length; i ++) {
        ...
        try {
            ...
            yield_opt(symbols[i], size).forEach((task) => kline_promises.push(request.send(task)));
            ...
            kline_results.forEach((kline_result, j) => {
                ...
                statistics_5[index + 1] = Number((total_5 * rate).toFixed(2));
                statistics_10[index + 1] = Number((total_10 * rate).toFixed(2));
                statistics_15[index + 1] = Number((total_15 * rate).toFixed(2));
                statistics_30[index + 1] = Number((total_30 * rate).toFixed(2));
