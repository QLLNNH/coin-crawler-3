...
exports.symbols_to_kline = async (symbols, interval) => {
    ...
    console.log(`完成指数请求${future_indexes.length}次`);

    const kline_promises = [];
    symbols.forEach((symbol) => yield_opt(symbol, size).forEach((task) => kline_promises.push(request.send_2(task))));
    // const kline_rets = await Promise.all(kline_promises);

    let kline_rets = [];
    for (let item = 0; item < kline_promises.length; item += 8) {
        const tmp = await Promise.all(kline_promises.slice(item, item + 8));
        kline_rets = kline_rets.concat(tmp);
    }

    console.log(`完成合约请求${kline_rets.length}次`);
